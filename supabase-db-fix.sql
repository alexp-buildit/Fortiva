...
DROP POLICY IF EXISTS "admin_transactions_all" ON transactions;
DROP POLICY IF EXISTS "users_own_transactions" ON transactions;
DROP POLICY IF EXISTS "users_create_transactions" ON transactions;
DROP POLICY IF EXISTS "users_update_transactions" ON transactions;

-- Admin users can see all transactions
CREATE POLICY "admin_transactions_all" ON transactions
  FOR ALL USING (
    EXISTS (
      SELECT 1 FROM users
